...
# ─── Leitura de todas as abas e concatenação ─────────────────────────────────
try:
...
col_navio1  = achar('navio / viagem.1','navio limpo')
col_navio0  = achar('navio / viagem','navio')
col_eta     = achar('estimativa chegada eta','eta')
col_etd     = achar('estimativa saída etd','etd')
col_movs    = achar('movs','teus','contêineres')
...
if col_eta:
    df_cancel[col_eta] = pd.to_datetime(
        df_cancel[col_eta], errors='coerce', dayfirst=True
    )
if col_etd:
    df_cancel[col_etd] = pd.to_datetime(
        df_cancel[col_etd], errors='coerce', dayfirst=True
    )

# ─── Campos derivados ─────────────────────────────────────────────────────────
df_cancel['Navio'] = df_cancel[
    col_navio1 or col_navio0
...
# ─── Cálculo de custos ─────────────────────────────────────────────────────────
C = {
    "TEU":1200, "OPER":1150, "DOC":950,
    "ARM_DIA":575, "ARM_DIAS":2, "INSP":95
}
if col_movs:
...
# ─── Função de tema Plotly ────────────────────────────────────────────────────
def theme_plotly(fig, height=450):
    fig.update_layout(
        height=height,
...
with tab1:
    ...
    c1.metric("Total registros", f"{len(df):,}", delta=f"{len(df_cancel):,}")
    ...
    # Pie Chart totalmente interativo
    fig = px.pie(
